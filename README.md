...
| **GPS 模拟** | 地图上点多个坐标、各设停留时长,循环发射让接收机位置按时间跳变 | `gps-sdr-sim` |
...
4. 点「生成并循环发射」:工具逐点用 gps-sdr-sim 合成信号,再按 1→2→3→… 顺序循环发射,让屏蔽箱里的接收机**位置按时间跳变**。发射时地图和坐标表锁定,当前坐标在表格里高亮,运行日志实时打印。
